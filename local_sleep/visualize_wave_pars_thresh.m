function [] = visualize_wave_pars_thresh(wave_pars, chan_locs)
...
topX_nums = [20, 15, 10, 5]; % top X % (i.e., 100 - rel. threshold)

% Iterate through different thresholds
for thr_ind = 1:length(wave_pars.threshs)
    
    top = topX_nums(thr_ind);

    topX_dir = sprintf('Topoplots/Top_%d', top);
    if ~exist(topX_dir, 'dir')
        mkdir(topX_dir);
    end
    
    field_names = fieldnames(wave_pars.threshs(thr_ind));
    ...
    for i = 1:length(field_names)
        ...
        filename = sprintf('%s/topoplot_%s_top_%d.png', topX_dir, par_name, top);
        saveas(gcf, filename);
    end
end
