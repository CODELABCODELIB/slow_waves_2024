...
window_starts = cell2mat(top10_filtered_results.channels(1).negzx) - 500;
...
% ch1_data = EEG.data(1,:);
ch1_data = preprocessed_EEG.data(1,:);
...
% for i = 1:num_windows
%     % waveforms(i,:) = ch1_data(window_starts(i):window_starts(i) + window_length - 1);
%     waveforms(i,:) = ch1_data(window_starts_indices(i):window_starts_indices(i) + window_length - 1);
% end

% Loop through each window and extract the data (for movie condition)
for i = 1:num_windows_movie
    % waveforms_movie(i,:) = ch1_data(window_starts_movie(i):window_starts_movie(i) + window_length_movie - 1);
    waveforms_movie(i,:) = ch1_data(window_starts_movie_indices(i):window_starts_movie_indices(i) + window_length - 1);
end

% Loop through each window and extract the data (for phone condition)
for i = 1:num_windows_phone
    % waveforms_phone(i,:) = ch1_data(window_starts_phone(i):window_starts_phone(i) + window_length_phone - 1);
    waveforms_phone(i,:) = ch1_data(window_starts_phone_indices(i):window_starts_phone_indices(i) + window_length - 1);
end
...
% xlabel('Time (s)');
% ylabel('Voltage (μV)');
% title('Average SW Waveform for Channel 1');
...
% title('Average SW Waveform for Channel 1', 'FontSize', 36);
title('Average SW Waveforms for Channel 1', 'FontSize', 36);
...
legend({'Movie (n = 701)', 'Phone (n = 437)'}, 'FontSize', 28);
