    function [twa_results]=twalldetectnew_TA_v3(datainput,orig_fs,thramp)
%% Delta/Theta Wave Detection
% 
% Code by Thomas Andrillon --> presumably used in Andrillon et al. (2021)
% Code modified by David Hof to perform the wave parameter extraction on 8
% equal-sized segments of the complete EEG data (--> temporal analysis).
%
...
segment_length = size(LoadedEEG.data,2) / num_segments; % Calculate length of each segment based on total data length
...
for s = 1:num_segments
    ...
    segmented_data{s} = LoadedEEG.data(:, start_index:end_index, :); % Segment data and store in cell array
end
...
for seg = 1:num_segments % Loop over segments
    ...
    for i = 1:size(current_segment,1) % Loop over channels within the current segment
        ...
        datax = squeeze(current_segment(i,1:size(current_segment,2),1));
