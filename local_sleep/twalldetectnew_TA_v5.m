    function [twa_results]=twalldetectnew_TA_v5(datainput,orig_fs,thramp)
...
%% Data Segmentation (added by David Hof, including code elements/adjustments further below to loop through segments)

num_segments = 8; % Define number of segments
segment_length = size(datainput,2) / num_segments; % Calculate length of each segment based on total data length
segmented_data = cell(1, num_segments); % Pre-allocate a cell array to store segmented data
for s = 1:num_segments
    start_index = round((s-1) * segment_length) + 1;
    end_index = round(s * segment_length);
    segmented_data{s} = datainput(:, start_index:end_index, :); % Segment data and store in cell array
end
...
for seg = 1:num_segments % Loop over segments
    % Modify to use current segment
    current_segment = segmented_data{seg};
    disp([' Analyzing Segment ', num2str(seg), '/', num2str(num_segments)]);

    % Initialize the segment and channel structure
    segments(seg).channels = repmat(struct(), size(current_segment,1), 1);

    disp([' Analyzing ', num2str(size(datainput,1)),' channels']);
    % h = waitbar(0,'Detection Progress');
    fprintf('\n');

    for i = 1:size(current_segment,1) % Loop over channels within the current segment
        fprintf('... channel %3.0f/%3.0f - %3.0f%%',i,size(datainput,1),0)
        % waitbar(i/size(LoadedEEG.data,1),h,'Detection Progress')
        % Data Extraction, Resample and Filtering
        EEG = squeeze(current_segment(i,1:size(current_segment,2),1));
	    % signal = resample(double(datax),fs,orig_fs);
        % EEG=filtfilt(bbp, abp, signal);
        datapoints=length(EEG);
        segments(seg).channels(i).datalength = datapoints;
    ...
    %% Wave parameters initialization %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	    segments(seg).channels(ch).negzx{1}=[];
	    segments(seg).channels(ch).poszx{1}=[];
	    segments(seg).channels(ch).wvend{1}=[];
	    segments(seg).channels(ch).negpks{1}=[];
	    segments(seg).channels(ch).maxnegpk{1}=[];
	    segments(seg).channels(ch).negpkamp{1}=[];
	    segments(seg).channels(ch).maxnegpkamp{1}=[];
	    segments(seg).channels(ch).pospks{1}=[];
	    segments(seg).channels(ch).maxpospk{1}=[];
	    segments(seg).channels(ch).pospkamp{1}=[];
	    segments(seg).channels(ch).maxpospkamp{1}=[];
	    segments(seg).channels(ch).mxdnslp{1}=[];
	    segments(seg).channels(ch).mxupslp{1}=[];
	    segments(seg).channels(ch).maxampwn{1}=[];
	    segments(seg).channels(ch).minampwn{1}=[];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
        %% Locate Peaks
	    wvi=1;
	    for wndx=start:length(negcross)-1
        fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b... channel %3.0f/%3.0f - %3.0f%%',i,size(datainput,1),round(100*wndx/(length(negcross)-1)))
    % 	disp([num2str(wndx)]);
    ...
    %% Wave parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        segments(seg).channels(ch).negzx{wvi}=round((single(wavest)./128).*orig_fs); % position of wave start (in ms)
        segments(seg).channels(ch).poszx{wvi}=round((single(poszx)./128).*orig_fs); % position of positive zero crossing (in ms)
        segments(seg).channels(ch).wvend{wvi}=round((single(wavend)./128).*orig_fs); % position of wave end (in ms)
        segments(seg).channels(ch).negpks{wvi}={round((single(bx)./128).*orig_fs)}; % position of neg peaks (in ms)
        segments(seg).channels(ch).maxnegpk{wvi}=round((single(maxbx)./128).*orig_fs); % position of max neg peak (in ms)
        segments(seg).channels(ch).negpkamp{wvi}=single(b); % neg peak amps
        segments(seg).channels(ch).maxnegpkamp{wvi}=single(maxb); % max neg peak amp
        segments(seg).channels(ch).pospks{wvi}={round((single(cx)./128).*orig_fs)}; % position of pos peaks (in ms)
        segments(seg).channels(ch).maxpospk{wvi}=round((single(maxcx)./128).*orig_fs); % position of max pos peak (in ms)
        segments(seg).channels(ch).pospkamp{wvi}=single(c); % pos peak amps
        segments(seg).channels(ch).maxpospkamp{wvi}=single(maxc); % max pos peak amp
        segments(seg).channels(ch).mxdnslp{wvi}=single(mxdn);
        segments(seg).channels(ch).mxupslp{wvi}=single(mxup);
        segments(seg).channels(ch).maxampwn{wvi}=single(maxampwn); % max pos amp in extended time window
        segments(seg).channels(ch).minampwn{wvi}=single(minampwn); % max neg amp in extended time window
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
