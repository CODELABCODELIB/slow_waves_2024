function [files_grouped,A,parfor_time] = call_f_all_p_parallel(path,f, options)
...
for i=1:length(files_grouped)
    ...
    if num_files > 1
        ...
        for j=1:num_files
            ...
            participant = sprintf('%s/%s',folder{i}, files_grouped{i}{1});
            if select_from_EEG_struct_3(EEG) && size(EEG.data,1) == 64
                [~,EEG] = f(EEG, participant, options); 
                EEG_all = [EEG_all, EEG];
            end
        end  
        if ~isempty(EEG_all)
    ...
    elseif num_files
        ...
        if mod(size(EEG.data,2), EEG.pnts) == 0 && ~(EEG.Attys) && size(EEG.data,1) == 64
