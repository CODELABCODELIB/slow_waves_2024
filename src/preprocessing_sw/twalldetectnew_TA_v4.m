...
fprintf('\n');
for i=1:size(datainput,1) % (i) is the number of the channel
...
%     waitbar(i/size(LoadedEEG.data,1),h,'Detection Progress')
    ...
    EEG = squeeze(datainput(i,1:size(datainput,2),1));
...
%% Wave parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    channels(ch).maxampwn{wvi}=single(maxampwn); % max pos amp in extended time window
    channels(ch).minampwn{wvi}=single(minampwn); % max neg amp in extended time window
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
