...
for K = K_range
    ...
    for r = 1:opts.reps
        ...
        if opts.optimised
            ...
            [A,L,Z,sig2,R2,MSE,ind,SSQ] = opt_segment(X,K,const1,opts);
        else
            % The original Basic N-Microstate Algorithm (Table I in [1])
            [A,L,Z,sig2,R2,MSE,ind,SSQ] = segmentation(X,K,const1,opts);
        end
...
function [A,L,Z,sig2,R2,MSE,ind,SSQ] = segmentation(X,K,const1,opts)
...
SSQ = mean((X-A*activations).^2);
end

function [A,L,Z,sig2,R2,MSE,ind,SSQ] = opt_segment(X,K,const1,opts)
...
function [L,sig2,R2,MSE,ind,SSQ] = smoothing(X,A,K,const1,opts)
